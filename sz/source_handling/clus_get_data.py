...
#  IF countnan GT 0 THEN mask[whnan] = 1
    ...
    maps = {'name':clusname, #clustername
          'file':file, #filename
          'band':band, #the name of the band (PSW, PMW, PLW)
          'signal':map.data, #the signal map
          'srcrm':srcrm, #the map with sources removed
          'xclean':xclean, #the map after we did cross correlation removal
          'error':err.data, #the error map
          'mask':mask, #mask map (it should be all zeros at this stage)
          'flag':flag, #mask map from the fits file
          'exp':exp, #exposure map
          'shead':head, #header for the primary image
          'ehead':herr, #header for the error image
          'astr':astr, #some astrometry parameters taken from primary header
          'pixsize':pixsize, #pixel size in arcseconds
          'psf':psf, #psf for each band
          'width':width, #fwhm in arcseconds
          'widtha':widtha, #fwhm in pixels
          'calfac':calfac, #a calibration factor
          'JY2MJy':config.JY2MJy} #a conversion factor
